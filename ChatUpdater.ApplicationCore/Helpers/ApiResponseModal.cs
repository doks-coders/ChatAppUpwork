...
namespace ChatUpdater.ApplicationCore.Helpers
{
    ...
    public sealed class ApiResponseModal<T> : ApiResponseModal
    {
        ...
        public static Task<ApiResponseModal<T>> SuccessAsync(T data)
            => Task.FromResult(new ApiResponseModal<T>(data, true));
        ...
        public new static async Task<ApiResponseModal<T>> FatalAsync(ApiErrorException exception)
        {
            Log.Fatal(exception, exception.ErrorCode);
            return await Task.FromResult(new ApiResponseModal<T>(false, exception.ErrorCode));
        }
